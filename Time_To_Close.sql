with Time_to_Close as(
  Select 
    ...
    contact_type,agent_site_group,login_name,INTE.routing_skill_cs_hierarchy2_name
	,routing_skill_name,to_skill_name,activity_week,activity_month,activity_date,
direction,agent_sic1_name,agent_sic2_name,agent_sic3_name,agent_sic4_name,comm_id,customer_id,resolved_contacts,time_to_closure,parked_contacts,handled_contacts
  from 
...
WHERE agent_site_group IN ('AMD','BBI','MLR','VTZ')
AND handled_contacts > 0 AND resolved_contacts >0
) 
select 
